...
pub fn sha1_compress_block(ctx: *Sha1Ctx) void {
    ...
    inline for (0..Sha1Ctx.MESSAGE_SCHEDULE_WORDS) |t| {
        const tmp = rotl(Sha1Ctx.WordType, a, 5) +% sha1_f(t, b, c, d) +% e +% sha1_k(t) +% message_schedule[t];
        e = d;
        d = c;
        c = rotl(Sha1Ctx.WordType, b, 30);
        b = a;
        a = tmp;
    }
...
fn sha2_compress_block(ctx: *Sha2Ctx) void {
    ...
    for (0..Sha2Ctx.BLOCK_SIZE / @sizeOf(Sha2Ctx.WordType)) |t|
        message_schedule[t] = deserialize_int_big_endian(
            Sha2Ctx.WordType,
    ...
    inline for (0..Sha2Ctx.MESSAGE_SCHEDULE_WORDS) |t| {
        const tmp1 = h +% sha2_Sigma_256_1(e) +% ch(Sha2Ctx.WordType, e, f, g) +% SHA2_K_256[t] +% message_schedule[t];
        const tmp2 = sha2_Sigma_256_0(a) +% maj(Sha2Ctx.WordType, a, b, c);
        h = g;
        g = f;
        f = e;
        e = d +% tmp1;
...
pub fn sha3_new(t: comptime_int) Sha3Ctx {
    const iv: *const [SHA_512_DIGEST_LENGTH / @sizeOf(Sha3Ctx.WordType)]Sha3Ctx.WordType = switch (t) {
        224 => &SHA_512_224_IV,
        256 => &SHA_512_256_IV,
        384 => &SHA_384_IV,
        512 => &SHA_512_IV,
...
pub fn sha3_update(ctx: *Sha3Ctx, message: []const u8) !void {
    return sha_generic_update(
        Sha3Ctx,
        1 << 128,
        sha3_compress_block,
...
pub fn sha3_final(ctx: *Sha3Ctx, digest_length: comptime_int, out: *[digest_length]u8) void {
    return sha_generic_final(
        Sha3Ctx,
        Sha3Ctx.WordType,
        u128,
...
fn sha3_compress_block(ctx: *Sha3Ctx) void {
    // Prepare the message schedule.
    var message_schedule: [Sha3Ctx.MESSAGE_SCHEDULE_WORDS]u64 = undefined;

    for (0..Sha3Ctx.BLOCK_SIZE / @sizeOf(Sha3Ctx.WordType)) |t|
        message_schedule[t] = deserialize_int_big_endian(
            Sha3Ctx.WordType,
            @ptrCast(ctx.message_buffer[(t * @sizeOf(Sha3Ctx.WordType))..((t + 1) * @sizeOf(Sha3Ctx.WordType))]),
        );
    for (Sha3Ctx.BLOCK_SIZE / @sizeOf(Sha3Ctx.WordType)..Sha3Ctx.MESSAGE_SCHEDULE_WORDS) |t| {
        message_schedule[t] = sha3_sigma_512_1(message_schedule[t - 2]) +% message_schedule[t - 7] +% sha3_sigma_512_0(message_schedule[t - 15]) +% message_schedule[t - 16];
    }
    ...
    // Perform the actual hashing.
    inline for (0..Sha3Ctx.MESSAGE_SCHEDULE_WORDS) |t| {
        const tmp1 = h +% sha3_Sigma_512_1(e) +% ch(Sha3Ctx.WordType, e, f, g) +% SHA3_K_512[t] +% message_schedule[t];
        const tmp2 = sha3_Sigma_512_0(a) +% maj(Sha3Ctx.WordType, a, b, c);
        h = g;
        g = f;
        f = e;
        e = d +% tmp1;
...
const SHA3_K_512 = [Sha3Ctx.MESSAGE_SCHEDULE_WORDS]u64{
...
fn sha3_Sigma_512_0(x: u64) u64 {
    return rotr(u64, x, 28) ^ rotr(u64, x, 34) ^ rotr(u64, x, 39);
}

fn sha3_Sigma_512_1(x: u64) u64 {
    return rotr(u64, x, 14) ^ rotr(u64, x, 18) ^ rotr(u64, x, 41);
}

fn sha3_sigma_512_0(x: u64) u64 {
    return rotr(u64, x, 1) ^ rotr(u64, x, 8) ^ shr(u64, x, 7);
}

fn sha3_sigma_512_1(x: u64) u64 {
    return rotr(u64, x, 19) ^ rotr(u64, x, 61) ^ shr(u64, x, 6);
}
...
pub fn sha384_update(ctx: *Sha3Ctx, message: []const u8) !void {
    if (dbg and !ctx.t_is_384)
        @panic("Debug: Attempt to call sha384_update on a SHA-3 context not initialized in 384-bit mode.");
    return sha3_update(ctx, message);
}

pub fn sha384_final(ctx: *Sha3Ctx, out: *[SHA_384_DIGEST_LENGTH]u8) void {
    if (dbg and !ctx.t_is_384)
        @panic("Debug: Attempt to call sha384_final on a SHA-3 context not initialized in 384-bit mode.");
    return sha3_final(ctx, SHA_512_DIGEST_LENGTH, out);
}
...
pub fn sha512_update(ctx: *Sha3Ctx, message: []const u8) !void {
    if (dbg and (ctx.t_is_224 or ctx.t_is_256 or ctx.t_is_384))
        @panic("Debug: Attempt to call sha512_update on a SHA-3 context not initialized in 512-bit mode.");
    return sha3_update(ctx, message);
}

pub fn sha512_final(ctx: *Sha3Ctx, out: *[SHA_512_DIGEST_LENGTH]u8) void {
    if (dbg and (ctx.t_is_224 or ctx.t_is_256 or ctx.t_is_384))
        @panic("Debug: Attempt to call sha512_final on a SHA-3 context not initialized in 512-bit mode.");
    return sha3_final(ctx, SHA_512_DIGEST_LENGTH, out);
}
...
pub fn sha512_224_update(ctx: *Sha3Ctx, message: []const u8) !void {
    if (dbg and !ctx.t_is_224)
        @panic("Debug: Attempt to call sha224_update on a SHA-3 context not initialized in 224-bit mode.");
    return sha3_update(ctx, message);
}

pub fn sha512_224_final(ctx: *Sha3Ctx, out: *[SHA_512_224_DIGEST_LENGTH]u8) void {
    if (dbg and !ctx.t_is_224)
        @panic("Debug: Attempt to call sha224_final on a SHA-3 context not initialized in 224-bit mode.");
    return sha3_final(ctx, SHA_512_DIGEST_LENGTH, out);
}
...
pub fn sha512_256_update(ctx: *Sha3Ctx, message: []const u8) !void {
    if (dbg and !ctx.t_is_256)
        @panic("Debug: Attempt to call sha512_256_update on a SHA-3 context not initialized in 256-bit mode.");
    return sha3_update(ctx, message);
}

pub fn sha512_256_final(ctx: *Sha3Ctx, out: *[SHA_512_256_DIGEST_LENGTH]u8) void {
    if (dbg and !ctx.t_is_256)
        @panic("Debug: Attempt to call sha256_final on a SHA-3 context not initialized in 256-bit mode.");
    return sha3_final(ctx, SHA_512_DIGEST_LENGTH, out);
}
...
test "SHA-512 padding test" {
    ...
    for (0..64) |i| {
        ...
        const ref = hex_to_bytes(SHA_512_DIGEST_LENGTH, reference[i]);
        try testing.expectEqualSlices(u8, ref[0..], digest_buffer[0..]);
    }
}
...
test "SHA-512 maximum length violation (simulated)" {
    ...
    try testing.expectError(MessageLengthLimitExceeded, sha512_update(&ctx, "a"));
}
